...
def parse_single_line(line: str, tz_offset: int = 3) -> Optional[Prediction]:
    ...
    # Clean up league: "Soccer. Brazil. Acreano U20" -> take last meaningful part
    ...
    # Split teams and bet
    # Bet is typically at the end: ф1-4,5 / П1 / ТБ2.5 / 1X etc
    # Teams separated by — or " - "
    bet = ''
    teams_part = after_time

    # Try to find bet at the end (after last space, no spaces inside bet usually)
    # Bet patterns: ф1-4,5  П1  ТБ2.5  ТМ1.5  1X  12  X2  ф2+2.5
    ...
    if bet_match:
        ...
        # Try last word as bet if it looks like one
    ...
    # Build match datetime for today
    today = date.today()
    try:
        match_time = datetime(today.year, today.month, today.day, hour, minute)
    except ValueError:
        return None
