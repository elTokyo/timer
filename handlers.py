...
from parser import parse_predictions, format_prediction_local as format_prediction
import storage
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, pred in enumerate(predictions, 1):
        lines.append(format_prediction(pred, s.timezone_offset, i))
        lines.append("")
...
async def list_predictions(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, pred in enumerate(predictions, 1):
        ...
        if pred.notified_5:
            ...
            status = " 🔔"
        lines.append(format_prediction(pred, s.timezone_offset, i) + status)
